---
title: 02. パーツデータ設計
description: 対象ブランド選定・データ収集方針・マスタスキーマ
sidebar:
  order: 2
---
...
国内で圧倒的なシェアを持つ主要ブランドに絞る。全網羅を狙わず、まず「定番」を押さえる。初期リリースの対象ブランドを以下のとおり**確定**する（#9）。

| ブランド | 主な強み | 優先度 | 主担当カテゴリ |
|---|---|---|---|
| **TRiNiDAD（トリニダード）** | 国内トップシェアのバレル | 最優先 | バレル |
| **TARGET（ターゲット）** | 海外大手・プロ使用多数 | 最優先 | バレル・シャフト・チップ |
| **COSMO DARTS（コスモダーツ）** | Fit Flight・バレル（DARTSLIVE 系） | 最優先 | バレル・シャフト・フライト・チップ |
| **L-style（エルスタイル）** | シャフト・フライトの定番（Lフライト） | 最優先 | シャフト・フライト・チップ |
| **Fit Flight（フィットフライト）** | フライトのデファクト規格（COSMO 系列） | 最優先 | シャフト・フライト |
| DYNASTY / A-FLOW | 人気バレルブランド | 初期収録（補完） | バレル |
| Harrows / unicorn | 海外定番 | 初期収録（補完） | バレル |

- **確定内容**: 最優先 5 ブランドを軸に、バレルの網羅性を高めるため DYNASTY / Harrows / unicorn も初期リリースに少数収録する。上記以外のブランドはローンチ後のフィードバック（#47）で追加する。
